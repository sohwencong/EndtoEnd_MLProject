...
def evaluate_models(X_train, y_train, X_test, y_test, models, param):
    try:
        ...
        for i in range(len(list(models))):  # loop through every model
            ...
            train_model_score = r2_score(y_train, y_train_pred)  # r2 score

            test_model_score = r2_score(y_test, y_test_pred)

            report[list(models.keys())[i]] = test_model_score  # append with every loop
...
def load_object(file_path):  # open and load object
    try:
